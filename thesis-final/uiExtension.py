...
def alerts_to_rows(alerts):
    ...
    for a in (alerts or []):
        try:
            ...
            param  = a.get("param", "")
            attack = a.get("attack", "")
            aiLabel = a.get("predicted_label", "")
            aiReason= a.get("reasoning", "")
...
def create_panel():
    ...
    # ให้ table ขยายเต็ม panel
    table.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS)
    table.setFillsViewportHeight(True)

    # ===== Column weight (สัดส่วน) =====
    # Risk, Alert, URL, CWE, Param, Attack, aiLabel, aiReason
    COLUMN_WEIGHTS = [1, 3, 4, 1, 2, 2, 1, 5]

    def resize_columns():
        total_weight = float(sum(COLUMN_WEIGHTS))
        total_width = table.getWidth()

        col_model = table.getColumnModel()

        for i in range(len(COLUMN_WEIGHTS)):
            width = int(total_width * (COLUMN_WEIGHTS[i] / total_weight))
            col_model.getColumn(i).setPreferredWidth(width)

    # เรียกตอนแรก
    resize_columns()

    # ให้ resize อัตโนมัติเมื่อ window เปลี่ยนขนาด
    class ResizeListener(ComponentAdapter):
        def componentResized(self, event):
            resize_columns()

    table.addComponentListener(ResizeListener())

    # ===== Sorting =====
    sorter = TableRowSorter(table_model)
    sorter.setComparator(0, RiskComparator())
    sorter.setComparator(3, IntLikeComparator())
    ...
    def run_scan(target_input):
        ...
        try:
            ...
            if alerts:
                ...
                cmd = ["/usr/bin/python3", "/mnt/shared/process.py"]
		process = subprocess.Popen(cmd)
		process.wait()

		if process.returncode != 0:
		    raise Exception("process.py failed")


                append_status("Reloading processed alerts...")

                final_path = "/mnt/shared/final.json"

		with codecs.open(final_path, "r", "utf-8") as f:
		    alerts = json.load(f)
